...
#[inline(always)]
pub fn inverse<'a, T: Num>(out: &'a mut [T; 3], a: &'a [T; 3]) -> &'a mut [T; 3] {
    out[0] = -a[0];
    out[1] = -a[1];
    out[2] = -a[2];
    out
...
#[inline(always)]
pub fn lerp<'a, T: Num>(out: &'a mut [T; 3], a: &'a [T; 3], b: &'a [T; 3], t: T) -> &'a mut [T; 3] {
    out[0] = a[0] + (b[0] - a[0]) * t;
    out[1] = a[1] + (b[1] - a[1]) * t;
    out[2] = a[2] + (b[2] - a[2]) * t;
    out
...
#[inline(always)]
pub fn min<'a, T: Num>(out: &'a mut [T; 3], a: &'a [T; 3], b: &'a [T; 3]) -> &'a mut [T; 3] {
...
#[inline(always)]
pub fn max<'a, T: Num>(out: &'a mut [T; 3], a: &'a [T; 3], b: &'a [T; 3]) -> &'a mut [T; 3] {
    out[0] = if b[0] > a[0] {b[0]} else {a[0]};
    out[1] = if b[1] > a[1] {b[1]} else {a[1]};
    out[2] = if b[2] > a[2] {b[2]} else {a[2]};
    out
...
#[inline(always)]
pub fn clamp<'a, T: Num>(out: &'a mut [T; 3], a: &'a [T; 3], min: &'a [T; 3], max: &'a [T; 3]) -> &'a mut [T; 3] {
